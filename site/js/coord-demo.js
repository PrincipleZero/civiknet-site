...
(function () {
  ...
  function createController(item) {
    ...
    if (state === 'reliable') {
      ...
      head.appendChild(svgEl('circle', { r: 11, fill: '#5cdb8e', opacity: 0.22 }));
      head.appendChild(svgEl('path', { d: 'M -8 -6 L 8 -6 L 0 9 Z', fill: '#5cdb8e', stroke: '#c5f5d8', 'stroke-width': 1, 'stroke-linejoin': 'round' }));
    ...
    function spawnScanRing(x, y) {
      var ring = svgEl('circle', { cx: x, cy: y, r: 8, fill: 'none', stroke: '#5cdb8e', 'stroke-width': 1.2, opacity: 0.55 });
